  ...
  test("captures emitted bundle files when Rolldown fails after writing output", async () => {
    ...
    try {
      ...
      expect(result.runtimeIdentity).toEqual({
        ...
        nodeOptionFiles: expect.any(Array) as unknown as never[],
        threadEnvironment: {
          NAPI_RS_ASYNC_WORK_POOL_SIZE: process.env.NAPI_RS_ASYNC_WORK_POOL_SIZE ?? null,
          ROLLDOWN_MAX_BLOCKING_THREADS: process.env.ROLLDOWN_MAX_BLOCKING_THREADS ?? null,
          ROLLDOWN_WORKER_THREADS: process.env.ROLLDOWN_WORKER_THREADS ?? null,
...
function testRuntimeIdentity(requestedPackageSpecifier = "rolldown"): ObservedRuntimeIdentity {
  return {
    ...
    nodeOptionFiles: [],
    threadEnvironment: {
      NAPI_RS_ASYNC_WORK_POOL_SIZE: null,
      ROLLDOWN_MAX_BLOCKING_THREADS: null,
      ROLLDOWN_WORKER_THREADS: null,
