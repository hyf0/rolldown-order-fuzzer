...
Each random-mixed case also rolls a **chunking-config axis** (`chunking:default` / `chunking:explicit` / `chunking:organic`), the per-case dimension for chunk-composition realism. `default` is rolldown's automatic chunking; `explicit` is the audited manual groups (exact module lists, including a cycle split across groups); `organic` is size/share-threshold groups whose composition ROLLDOWN decides — mapping onto its `output.codeSplitting.groups` with randomized `minShareCount` (vendor-style merges / high-in-degree shared chunks), `minSize`/`maxSize` (byte-driven splitting), `priority` (competing groups), and an optional `\.mjs$`/`\.cjs$` regex `test` (format-vendor merges). An end-stage factor inspired by the execution-order failure reported in rolldown#10259 replaces the chunk config of an eligible strict ESM source DAG with one exact manual group: `entriesAware` plus a large merge threshold co-locates two effectful entry-private app modules while an effectful private side-effect dependency stays outside, so Rolldown can manufacture a common-chunk ↔ entry-chunk cycle. `variation:entries-aware-group` records the option and `mechanism:merged-entry-group-init-cycle` records the structural recipe; a fixed three-entry builder and `vp run entries-aware-cycle-catch` prove the released `init_* is not a function` failure and the declaration-form wrapper fix in both on-demand and wrap-all strict modes. Chunking is bundle-side only, so it never changes source-run semantics and the oracle stays valid; the chosen config is recorded in `model.json` and replays byte-identically. Dynamic-import density is raised and some dynamic edges deliberately originate from a module reachable only via a dynamic import — a dynamic import inside a dynamically-imported module (`mechanism:nested-dynamic`), the dense route-splitting shape real apps have. See [.agents/docs/organic-chunking-and-scale.md](.agents/docs/organic-chunking-and-scale.md) and [.agents/docs/entries-aware-chunk-cycle.md](.agents/docs/entries-aware-chunk-cycle.md).
