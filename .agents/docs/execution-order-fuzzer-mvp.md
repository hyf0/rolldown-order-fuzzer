...
The adapter does not enable Rolldown devtools and does not read internal wrapping, inclusion, or execution-plan state. Artifact schema 7 records the generated model, manifests, observed source and bundle outcomes, emitted bytes, exact verdict, and tested runtime/package identity, including package source files, direct runtime dependencies, and NAPI binding-selection environment. The adapter compares that identity before and after each build. The differential source-versus-bundle execution result is the sole semantic oracle.
